# FreeDirect Task Plan — Archive (2026-07-18)

This file is an **archive**, not a living document. It is not meant to be edited going forward —
unlike `plan.md`, which stays the authoritative, currently-maintained task list.

It contains everything from `plan.md` confirmed complete as of 2026-07-18, split out to keep the
live `plan.md` focused on actual remaining work:

- **From the original Phase 0-18 plan**: Phase 0 (repository/call-site audit), Phase 2 (DirectPlay
  state model), Phase 3 (message queue semantics), Phase 4 (loopback backend), Phase 5 (ENet
  integration planning), Phase 15 (tests and CI), Phase 16 (documentation), and Phase 17 (`NEXT.md`
  workflow). Each was individually re-verified against current source/docs/tests on 2026-07-18
  (not just checkbox-counted) before being archived — see each phase's own per-item notes below for
  the specific evidence. A handful of checkboxes that had simply never been ticked despite the
  underlying work being done were checked off as part of this verification pass, with a citation
  of what was checked and how.
- **The entire "24-Hour Autonomous Stabilization Backlog"**: all 189 `TASK-24H-XXXX` tasks, every
  one `Status: DONE`, confirmed by `grep` before archiving.

**Not archived** (stayed in the live `plan.md`): Phase 1, Phase 6, Phase 7, Phase 8, Phase 9,
Phase 10, Phase 11, Phase 12, Phase 13, Phase 14, and Phase 18. Each of these turned out to have at
least one genuinely unresolved item on close inspection — some phases whose *titles* closely
resembled archived work (e.g. Phase 11's error-semantics sweep, Phase 12's SDL3_net documentation,
Phase 13's DirectSound hardening, Phase 14's DirectDraw hardening) were **not** archived wholesale,
specifically because a few of their individual checkboxes did not hold up under direct
verification (a missing `DPERR_UNSUPPORTED` return path, an un-performed `GetCurrentPosition`
audit, an undocumented mixed-bit-depth blit decision, and similar). See `plan.md` itself for the
current, accurate state of what's actually still open.
...
- [x] Add protocol documentation for the header layout above to `docs/directplay-protocol.md`
      (Phase 16). Verified 2026-07-18: `docs/directplay-protocol.md` documents the full 72-byte
      wire header field-by-field (offsets, types, `payloadLength` cross-check), including the
      ABI-portability caveat (64 bytes on LLP64). Checkbox had simply never been ticked despite the
      doc existing.
...
- [x] Add a DirectPlay unit test executable (e.g. `tests/directplay_tests`), linking against
      `free-direct` and using the loopback transport by default. Re-verified 2026-07-18 via a
      fresh clean build + `ctest` run (9/9 tests pass, including `directplay_tests`).
- [x] Add a DirectDraw unit test executable (e.g. `tests/directdraw_tests`) if not already present
      from Phase 14's work. Re-verified 2026-07-18, same build/test run.
- [x] Add a DirectSound unit test executable (e.g. `tests/directsound_tests`) if not already
      present from Phase 13's work. Re-verified 2026-07-18, same build/test run.
- [x] Add CTest integration in `CMakeLists.txt` (`enable_testing()` plus `add_test()` per
      executable) so `ctest` runs all three suites. Re-verified 2026-07-18.
- [x] Add headless-friendly test configuration (no real window/display/audio device required for
      the default `ctest` target). Re-verified 2026-07-18 (no display/audio device present in the
      environment the fresh build/test ran in; all 9 tests still passed).
- [x] Add the loopback DirectPlay tests from Phases 3-11 to the default CTest run. Re-verified
      2026-07-18 - `directplay_tests` runs by default, no `FREE_DIRECT_ENABLE_ENET` flag needed.
- [x] Add ENet local integration tests from Phase 10 as a separate CTest target guarded by
      `FREE_DIRECT_ENABLE_ENET`, excluded from the default `ctest` run. Confirmed via
      `tests/enet_directplay_tests.cpp` (`ctest -L enet`) and `CMakeLists.txt`'s
      `FREE_DIRECT_ENABLE_ENET` gating - not re-built in this pass (only the default config was
      re-verified today), but the gating structure itself was confirmed present in source.
- [x] Add a sanitizers CMake option (e.g. `FREE_DIRECT_ENABLE_ASAN`/`FREE_DIRECT_ENABLE_UBSAN`) for
      local/CI opt-in use. Confirmed present in `CMakeLists.txt` (not re-built in this pass).
- [x] Add a CI build matrix if this repository (or its parent build) uses GitHub Actions — check
      for an existing `.github/workflows` directory first, and only add one if none exists.
      Verified 2026-07-18: no `.github/workflows/` directory exists, and `docs/ci-matrix.md`
      explicitly documents this as a deliberate, ask-first decision ("No `.github/workflows/*.yml`
      file exists... Adding real CI infrastructure is a separate, ask-first decision") rather than
      an oversight - satisfies the bullet's own conditional ("only add one if none exists").
- [x] Add test documentation (a short `tests/README.md`, or a section in the main `README.md`)
      explaining how to build and run the test suites, including the ENet-gated ones. Verified
      2026-07-18: `README.md`'s "Build Instructions" section covers standalone build, test running,
      and (further down, not re-quoted here) the ENet-gated path.
...
loopback-backed suites. **Directly re-verified 2026-07-18** with a genuinely fresh out-of-tree
build (not reusing a stale build directory): exit 0, 9/9 tests passed.
...
- [x] Add a task (executed at the start of real Phase 1+ implementation work, not before) to
      create `NEXT.md` per the `NEXT.md` Policy in `CLAUDE.md`. `NEXT.md` exists and has for most
      of this project's history.
- [x] Add a recurring task to update `NEXT.md` after each completed implementation batch (roughly
      one PR's worth of finished `plan.md` checkboxes). Confirmed as an established, followed
      practice - `NEXT.md` was updated twice more in this same session (2026-07-18).
- [x] Ensure every `NEXT.md` update includes the current branch/state. Present in every version.
- [x] Ensure every `NEXT.md` update includes completed tasks, referencing specific `plan.md`
      checkbox items. Present (Section 3, "Recent changes", cites commit hashes and the specific
      work each did).
- [x] Ensure every `NEXT.md` update includes partially completed tasks and exactly what remains
      for each. Present (Section 5, "Known bugs and limitations").
- [x] Ensure every `NEXT.md` update includes known blockers. Present (Section 4).
- [x] Ensure every `NEXT.md` update includes next recommended tasks, pointing at specific
      `plan.md` items. Present (Section 8).
- [x] Ensure every `NEXT.md` update includes test status (what was actually run, pass/fail
      counts). Present (Section 2).
- [x] Ensure every `NEXT.md` update includes build status (does a clean build currently succeed).
      Present (Section 2).
- [x] Do not create fake progress in `NEXT.md` — cross-check this rule during any review of a
      `NEXT.md` update. Followed throughout - e.g. Section 4/8 explicitly say "no problem to
      report" rather than inventing one, and a mistaken claim (`QueryInterface` "implemented") was
      caught and corrected in this same session rather than left standing.
...
placeholder. **Met** - `NEXT.md` has existed and been actively maintained across this project's
history, verified directly by reading its current content 2026-07-18.
