/**
 * @file DirectPlayInternal.hpp
 * @brief Internal DirectPlay shared state: debug logging helpers, GUID/session-registry helpers,
 *        and the `DirectPlay2AImpl` class declaration (full definition - see below).
 *
 * This header is intentionally private to the DirectPlay implementation: it must never be
 * included from `include/dplay.h` and must never be installed. `DirectPlay2AImpl` (the real
 * `IDirectPlay2A` implementation, the dominant class in this subsystem) is defined here in full
 * (all methods inline in the class body, exactly as it always has been - `src/directplay/`'s
 * other state-holder headers, e.g. `DirectPlaySession.hpp`, already use this same header-only
 * pattern, paired with a trivial `.cpp` that just includes the header to give it a translation
 * unit/CMake source entry) rather than split into declarations-here/bodies-in-a-.cpp, unlike the
 * `src/directdraw/` split done earlier the same day - that split's classes were already
 * out-of-line before the split; `DirectPlay2AImpl`'s methods were not, and converting ~30
 * methods from inline-in-class to out-of-line-with-`ClassName::`-qualification purely for
 * stylistic parity would have been unnecessary rewrite risk for a purely organizational move.
 * `DirectPlayImpl` (the small `IDirectPlay` COM-boundary object) stays in `DirectPlay.cpp`
 * itself, alongside the free functions (`DirectPlayCreate`, `DirectPlayEnumerateA`/`W`) it is
 * tightly coupled to - see that file for it.
 *
 * Every test exercises `DirectPlay2AImpl` only through the real public `IDirectPlay2A` interface
...
namespace free_direct_directplay {
    ...
    inline bool IsDirectPlayDebugEnabled()
    {
    ...
    inline void DirectPlayLog(const char* format, ...)
    {
    ...
    inline GUID GenerateSessionInstanceGuid() {
    ...
    inline bool ParseEnetHostAddressEnvVar(std::string& outHost, std::uint16_t& outPort) {
    ...
    inline std::unordered_map<std::uint16_t, free_direct_directplay::DirectPlaySession*>&
    LoopbackHostedSessionRegistry() {
        static std::unordered_map<std::uint16_t, free_direct_directplay::DirectPlaySession*> registry;
        return registry;
    }

    inline void UnregisterHostedSession(free_direct_directplay::DirectPlaySession* session) {
